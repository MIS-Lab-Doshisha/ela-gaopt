"""
前処理済みデータの二値化スクリプト   (HCPデータ用)
----------------------------------------------------
動作確認済み/Tested with Python 3.13.2
------------------------------------
Script for binarizing preprocessed data (HCP data)
--------------------------------------------------
"""
...
TR = 0.72                                    # TR (秒)/ TR (seconds)
radius = 4                                   # ROIの半径 (mm)/ ROI radius (mm)
raw_site_name = [""]             # 生データのサイト名リスト / List of raw data site names
session_list  = ["LR_run-01"]#,"LR_run-02","RL_run-01","RL_run-02"]                         # セッションリスト / List of sessions

# --- パス設定 / Path settings ---
subjects_path = "Data//preprocessed//HCP//derivatives//"
behavioral_path = ""            # 行動データのパス / Path to behavioral data
pkl_path = "Data//test_data_4//"                   # 出力先のパス / Output path
...
rois = pd.concat([rois_x,rois_y],axis=1)                       # ROIの座標を結合 / Concatenate ROI coordinates
...
global count_extract                                           # 抽出した時系列のカウント / Count of extracted time series
count_extract = 0                                              # 抽出した時系列のカウントを初期化 / Initialize count of extracted time series
...
def binarize(TimeSeries_list,binarize_list):
    for i in range(len(TimeSeries_list)):
        ...
        binarize_list.append(binarize_TimeSeries)                                                   # 二値化したデータをリストに追加 / Append the binarized data to the list    
    ...
    for subject in subject_list:
        for session in session_list:
            ...
            if session == "":
                ...
                bold_file = f'{subjects_location}sub-{subject}/func/sub-{subject}_task-REST_acq-{session}_space-MNI152NLin2009cAsym_res-2_desc-preproc_bold.nii.gz'           
            ...
            count_extract += 1
            print(f"Extracted time series count: {count_extract}")
...
def main():
    for site_name in raw_site_name:
        ...
        # HCPデータの被験者IDリスト / List of subject IDs for HCP data
        subject_list =[
        "100206"]
        ...
        f = open(f'{pkl_path}test_data_run01_500_5.pkl',"wb")
