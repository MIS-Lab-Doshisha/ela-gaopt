"""
ROIごとの局所安定状態頻度算出スクリプト
----------------------------------------------------
動作確認済み/Tested with Python 3.13.2
------------------------------------
GAで選択された各個体（ROIセット）について、全被験者の脳活動データから
局所安定状態（local minima）の出現頻度を計算し、結果をpklファイルで保存します。
...
def func_ELA(individual, task_data_train):
    """
    各個体（ROIセット）に対して局所安定状態の出現頻度を計算
    Calculate frequency of local minima for each individual (ROI set)
    """
    # 選択ROIのマスクを作成 / Create mask for selected ROIs
    selection_mask = np.array(individual, dtype=bool)
    # 各被験者のデータから選択ROIのみ抽出 / Extract only selected ROIs for each subject
    filter_list = [data[selection_mask].astype('int32') for data in task_data_train]
    # 全被験者分を列方向に連結 / Concatenate all subjects' data column-wise
    ...
    # 局所安定状態頻度を計算 / Calculate local minima frequency for each subject
    ...
    for i, filtered in enumerate(filter_list):
        ...
        # 出現回数カウント / Count occurrences
        counts = {}
        for num in localmin_list:
            counts[num] = counts.get(num, 0) + 1
        # 出現頻度計算 / Calculate frequency
...
def main():
    ...
    ga_result_path = "ELAGAopt_result//GA_result//best_individual//scenario_2"
    output_path = "ELAGAopt_result//Analysis_result//local_min_freq_data//freq_ROI13.pkl"

    # 脳活動データの読み込み / Load brain activity data
    task_data_all,_ = load_brain_data(pkl_path, group_split=False)

    freq_list = []
    n_individuals = 100  # 個体数 / Number of individuals
    for k in range(n_individuals):
        # GAで選択された個体データの読み込み / Load GA-selected individual
        ...
        # 局所安定状態頻度を計算 / Calculate local minima frequency
    ...
    # 結果をpklファイルで保存 / Save results as pkl file
