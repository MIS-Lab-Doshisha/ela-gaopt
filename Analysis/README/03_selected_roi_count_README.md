...
## 1. Inputs and default paths (as in script)
...
## 3. Detailed workflow (what `main()` does)

1. Instantiate `ROISelectionAnalyzer` with `n_trials=N` and `expected_p=13/264`.
2. Call `collect_roi_selection_labels()` to read each GA `best_ind_1000_all_ROI13_{idx}.csv`, extract the final-generation individual, map selected bits to labels from the atlas label file, and save an aggregated CSV of selected labels across runs.
...
## 4. Statistical formulas (reference)
